...
module.exports = (server) => {
  ...
  /*   socket.on('check-card', (player, target, cb) => {
    ...
    /*  setTimeout(() => {
      ...
      var voteTick = () => {
        ...
        if(timeLeft > 0) {
          ...
          let log = powerup.getVoteLog()
          io.emit('vote-stop', log)
        }
      }
      voteTick()
    }

    socket.on('vote-start', () => timeLeft = Math.max(0, timeLeft - 5000))
    ...
    socket.on('powerup-use', (name, target, cb) => {
      console.log('powerup-use ' + name + ' for ' + socket.user + ' target ' + target)
      powerup.activate(socket.user, name, target)
