...
var secretStrings = [
  ...
  { secret: "Boîte de Pandore", hint: "Boîte de Pandore"},
  { secret: "Epée de Damoclès", hint: "Epée de Damoclès"},
  { secret: "Mjöllnir, le marteau de Thor", hint: "Mjöllnir, le marteau de Thor"},
...
var teamStrings = ["Illuminati", "Scientolog"]
...
var soloKillOneCount = 3

var soloLoseCount = 4

const secretPlayerCount = 5
...
function getTeamSecret(team, exclude) {
  ...
  while(!output || output === exclude) {
    output = players.getData(getTeamPlayer(team), "secrets")[randomRange(secretPlayerCount)].secret.hint
  }
  return output
}

function buildRevelationHasExactlyOne(team) {
  let secrets = []
  secrets[0] = getTeamSecret(team)
  secrets[1] = getTeamSecret(getOtherTeam(team), secrets[0])
  shuffle(secrets)
  let output = team + " ont " + secrets[0] + " SOIT " + secrets[1]
  console.log(output)
  return output
}

function buildRevelationHasAtLeast(team) {
  let secrets = []
  secrets[0] = getTeamSecret(team)
  ...
  shuffle(secrets)
  let output = team + " ont " + secrets[0] + " ET/OU " + secrets[1]
  console.log(output)
  return output
}
...
function buildSoloSecrets (names, secretCount) {
  names.forEach ((name) => {
    ...
    players.listPlayerNames().forEach(other => {
      ...
      let secret = secrets[Math.trunc(Math.random() * 5)]
...
function start () {
  ...
  secrets = _.concat(secretStrings).slice(0,teamNames.length * 5)
  soloNames = []
  ...
  buildTeamSecrets(teams, knowledgePlayerCount, 5)
  ...
  let knowledges = [
    buildRevelationHasExactlyOne(teamStrings[0]),
    buildRevelationHasExactlyOne(teamStrings[1])
  ]
