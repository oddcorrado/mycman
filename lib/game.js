...
function buildRevelationLeader() {
...
function buildRevelation(team) {
  if(!team) {
    return buildRevelationLeader()
  }
  if(Math.random() > 0.5) {
    return buildRevelationHasAtLeast(team)
  } else {
    return buildRevelationHasExactlyOne(team)
  }
}
...
function start () {
  ...
  // TODO check solo
  // TODO knowledege count is 0

  let knowledges = [
    buildRevelation(teamStrings[0]),
    buildRevelation(teamStrings[1])
  ]

  if(isLeaderMode) {
    knowledges.push(buildRevelation(null))
  }
...
function voteStop(targets) {
  ...
  let knowledge = buildRevelation(teamStrings[revelationTeamIndex])
