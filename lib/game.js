...
let isAutoHint = false
let hintHistory = []
let useTeamCommonSecrets = false
let hints = []
...
const soloKillOneCount = 3

const storyTypes = ['place', 'action', 'item']

const soloGoalTexts = {
  soloRevealOneSolo:'Se faire Révéler tous ses secrets',
...
// it returns a secret
function pickSecret (name, type, storyType) {
  // console.log("PICK SECRET", name, storyType)
  ...
  if(storyType) {
    filteredSecrets = filteredSecrets.filter(v => storyType === v.storyType)
  }
...
function addKnowledge (name, knowledge) {
  ...
  if(knowledge.id != null && knowledges.find(k => k.id === knowledge.id)) {
    return
  }
...
function buildHint(team) {
  let text = null
  if (!team) {
    text = buildHintLeaderNegative()
  } else {
    text = buildHintDirect(team)
  }
  if(!text) {
    return null
  }
  let id = hints.length + 1
  hints.push({text, id})
  return({text, id})
}
...
function buildSoloSecrets (names, secretCount) {
  names.forEach ((name) => {
    ...
    for (let j = 0; j < secretCount; j++) {
      ...
      let secret = pickSecret(name, null, storyTypes[j])
      addSecret(name, {secret, isShared:false})
    }
...
function buildTeamLeaders (teams) {
  ...
  teams.forEach((team, i) => {
    ...
    team.forEach ((name, i) => {
      ...
      players.setData(name, "solo", false)
    })
  })
}

function buildSolos (solos) {
  solos.forEach(solo => {
    players.setData(solo, "solo", true)
  })
}
...
//  buildTeamSecrets(teams, knowledgePlayerCount, gameOptions.secretPlayerCount
  ...
  if(useTeamCommonSecrets) {
    knowledges = [
      // {id:null, text:'Les ' + teamStrings[0] + ' ont ' + teamCommonSecrets[0].secret},
      // {id:null, text:'Les ' + teamStrings[1] + ' ont ' + teamCommonSecrets[1].secret},
    ]
  } else {
    knowledges = [
  ...
  if(isLeaderMode) {
    if(isAutoHint) {
      ...
      while(buildHint(null)) {
        console.log(unusedNonLeaderSecrets.length)
      }
      for(let i = 0; i < teams[0].length * gameOptions.secretPlayerCount; i++) {
        for(let j = 0; j < teamCount; j++) {
          buildHint(teamStrings[j])
        }
      }
      shuffle(hints)
  ...
  names.forEach(name=>{
    ...
    if(gameOptions.numberOfChecksAtStart === -1) {
      ...
      for(let i = 0; i < gameOptions.numberOfChecksAtStart; i++) {
        players.addPendings(name, '*')
      }
    }
...
function getData (keys, playerName) {
  ...
  case "player": {
    ...
    let isSolo = players.getData(name, "solo")
    console.log('>>>', name, isSolo, isLeader)
    let data = {
      team: isSolo ? players.getData(name, "team") : (isLeader ? "Elu" : players.getData(name, "team")),
  ...
  case "hint": {
    let id = keys[1]
    console.log("GET HINT", id)
    let hint = hints.find(hint => hint.id === id)
    if(hint) {
      addKnowledge (playerName, hint)
    }
    return hint
  }
  case "word": {
...
function voteStop(targets) {
  ...
  names.forEach(name => {
    for(let i = 0; i < gameOptions.numberOfChecksPerTurn; i++) {
      players.addPendings(name, '*')
    }
    players.checkPendings(name)
    if(knowledge) {
...
function checkVictoryLeader()
{
  ...
  teams.forEach((team, i) => {
    ...
    teamLeaderCount[i] = secrets.filter(v => v.isRevealed).length

    if (teamLeaderCount[i] >= gameOptions.secretPlayerCount) {
      isGameOver = true
    }

    team.forEach(name => {
      let secrets = players.getData(name, "secrets")
      teamRevealCount[i] = (teamRevealCount[i] || 0 ) + secrets.filter(v => v.isRevealed).length
    })

    console.log("team", team, i, teamLeaderCount[i], teamRevealCount[i], isGameOver)
  })

  if(isGameOver) {
    teamStrings.forEach((name, i) => log.scoresAdd(name, teamLeaderCount[i], teamRevealCount[i]))
    let winners = []
    if(soloWin) {
      winners = -1
    } else {
      // first check leaders to find winning team
      teamLeaderCount.forEach((count, i) => {
        if (count < gameOptions.secretPlayerCount) {
          isGameOver = true
          winners.push(i)
        }
      })
      // if none is better check reveal count
      if(winners.length === 0) {
        let smallest = teamRevealCount.concat().sort((a,b) => (a-b))[0]
        let cnt = teamRevealCount.filter(v => v === smallest).length
        if(cnt === 1) {
          winners.push(teamRevealCount.findIndex(v => v === smallest))
        }
      }
    }
    gameOver(winners)
...
function checkVictoryCount()
{
  ...
  teams.forEach((team, i) => {
    ...
    team.forEach(name => {
      let secrets = players.getData(name, "secrets")
      teamRevealCount[i] += secrets.filter(v => v.isRevealed).length
    })
  })

  teamRevealCount.forEach((count, i) => {
    if (count >= loseCount) {
      isGameOver = true
...
function decisionStop(decisions) {
  ...
  var teamRevealCount = Object.keys(decisions).reduce((total, name) => decisions[name].y > 325 ? total + 1 : total, 0)
  var moneyUpdate = 0
  if(bankCount > 0) {
    moneyUpdate = Math.trunc(bank/bankCount)
  }
  var revealCost = teamRevealCount */
  var checks = []
  var names = players.listPlayerNames()
...
//  setBank(bank - moneyUpdate * bankCount + revealCost * teamRevealCount)
  // console.log(checks)
}
