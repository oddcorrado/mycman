...
function activateHack(target, source, type, isTarget, cost, duration) {
  ...
  var hack = {origin:source, name:isTarget ? source : target, left:duration}
...
function action (keys, playerName) {
  switch(keys[0]) {
  case "hack-jam": {
    return activateHack(keys[1], playerName, "jammers", true, 3, 15000)
  }
  case "hack-spy": {
    return activateHack(keys[1], playerName, "spies", true, 3, 15000)
  }
  case "hack-usurp": {
    return activateHack(keys[1], playerName, "usurpators", false, 1, 5000)
  }
  }
}
...
function mp (player, target, message) {
  ...
  if(playerHacks && playerHacks.mp && playerHacks.mp.usurpators && playerHacks.mp.usurpators.length > 0) {
    console.log(playerHacks.mp.usurpators)
