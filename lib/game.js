...
// it returns a secret
function pickSecret (type) {
  let filteredSecrets = secrets
  if(type) {
    filteredSecrets = filteredSecrets.filter(v => type === v.secret.type)
    console.log('####################', filteredSecrets)
  }
  let s = Object.assign({} , filteredSecrets[Math.trunc(Math.random() * secrets.length)])
...
function buildTeamSecretsCommonSecret (teams, secretCount) {
  teams.forEach ((team, teamIndex) => {
    teamCommonSecrets[teamIndex] = pickSecret()
    // add secrets
    team.forEach ((name) => {
      let commonSecret = Object.assign({}, teamCommonSecrets[teamIndex])
      gameSecrets.push(commonSecret)
      addSecret(name, {secret:commonSecret, isRevealed:false})
      players.setData(name, "team", teamStrings[teamIndex])
