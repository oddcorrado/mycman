const secretStrings = [
  ...
  { secret: "Le baton de Moïse", hint: "Le baton de Moïse", cooldown:0, cooldownPeriod:4, inUse:false, help:'Les joueurs ayant 0 votes sont révélés', log:'log text', targetMax:0, targetNoSelf:false,  targets:[], available:false, pos:10},
  { secret: "Le Golem", hint: "Le Golem", cooldown:0, cooldownPeriod:3, inUse:false, help:'Donne +3 au vote à un autre joueur', log:'log text', targetMax:0, targetNoSelf:false,  targets:[], available:false, pos:10},
  ...
  { secret: "La Corne d'Abondance", hint: "LLa Corne d'Abondance", cooldown:0, cooldownPeriod:4, inUse:false, help:'Les joueurs ayant 1 vote sont révélés', log:'log text', targetMax:0, targetNoSelf:false,  targets:[], available:false, pos:10},
...
secretStrings.find(s => s.secret === 'Le baton de Moïse').voteDecide = function({counts, decisions, log}) {
  counts.filter(count => count.count === 0).forEach(count => {
    let decision = decisions.find(decision => count.name === decision.name)
    if(decision && !decision.isRevealed) {
      decision.isRevealed = true
      log += 'Le baton de Moïse '+ decision.name + '\n'
    }
  })
  return {counts, decisions, log}
}
