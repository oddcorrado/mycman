...
const secretStrings = [
  ...
  // MP LOG
  { secret: "L'ATLANTIDE", storyType: "place", type: "spy", short:'espion', hint: "L'ATLANTIDE", cooldown:0, cooldownPeriod:0, inUse:false, help:'Récupère les 20 derniers messages', targetMax:1, targetNoSelf:true,  targets:[], available:false, pos:10},
  ...
  { secret: "LE MARTEAU DE THOR", storyType: "item", type: "spy", short:'espion', hint: "LE MARTEAU DE THOR", cooldown:0, cooldownPeriod:0, inUse:false, help:'Récupère les 40 derniers messages échangés par un joueur', targetMax:1, targetNoSelf:true,  targets:[], available:false, pos:10},
  { secret: "LE NECRONOMICON", storyType: "item", type: "spy", short:'espion', hint: "LE NECRONOMICON", cooldown:0, cooldownPeriod:0, inUse:false, help:'Récupère les 20 derniers messages', targetMax:1, targetNoSelf:true,  targets:[], available:false, pos:10},
  ...
  // BOMBS
  { secret: "LE YETI", storyType: "item", type: "bomb", short:'révèle 0', hint: "LE YETI", cooldown:0, cooldownPeriod:100, inUse:false, help:'Les joueurs ayant 0 votes sont révélés', log:'Les joueurs ayant 0 votes ont été révélés', targetMax:0, targetNoSelf:false,  targets:[], available:false, pos:10},
  { secret: "LE GOLEM", storyType: "item", type: "bomb", short:'révèle 1', hint: "LE GOLEM", cooldown:0, cooldownPeriod:100, inUse:false, help:'Les joueurs ayant 1 vote sont révélés', log:'Les joueurs ayant 1 vote ont été révélés', targetMax:0, targetNoSelf:false,  targets:[], available:false, pos:10},
  ...
  { secret: "LA MOMIE", storyType: "item", type: "bomb", short:'inverse', hint: "LA MOMIE", cooldown:0, cooldownPeriod:4, inUse:false, help:'Inverse les révélés', log:'Inverse les révélés', targetMax:0, targetNoSelf:true,  targets:[], available:false, pos:10},
...
// JAM
// *******************************************************
secretStrings.find(s => s.secret === 'L\'ATLANTIDE').lifecycleActivate = function() {
...
secretStrings.find(s => s.secret === 'LE MARTEAU DE THOR').lifecycleActivate = function() {
  return log.messagesAll().filter(v => (v.from === this.targets[0] || v.to === this.targets[0])).slice(-40)
}

secretStrings.find(s => s.secret === 'LE NECRONOMICON').lifecycleActivate = function() {
  return log.messagesAll().slice(-20)
}
...
// BOMBS
// *******************************************************
secretStrings.find(s => s.secret === 'LE YETI').voteDecide = function({counts, decisions, log}) {
  counts.filter(count => count.count === 0).forEach(count => {
    ...
    if(decision && !decision.isRevealed) {
      ...
      log += 'LE YETI révèle '+ decision.name + '\n'
    }
  })
  return {counts, decisions, log}
}

secretStrings.find(s => s.secret === 'LE GOLEM').voteDecide = function({counts, decisions, log}) {
  counts.filter(count => count.count === 1).forEach(count => {
    let decision = decisions.find(decision => count.name === decision.name)
    if(decision && !decision.isRevealed) {
      decision.isRevealed = true
      log += 'LE GOLEM révèle '+ decision.name + '\n'
    }
  })
  return {counts, decisions, log}
}
...
secretStrings.find(s => s.secret === 'LA MOMIE').voteDecide = function({counts, decisions, log}) {
  decisions.forEach(decision => {
    if (decision.isRevealed === true) {
      decision.isRevealed = false
      log += 'LA MOMIE cache '+ decision.name + '\n'
    } else {
      decision.isRevealed = true
      log += 'LA MOMIE révèle '+ decision.name + '\n'
    }
  })
  return {counts, decisions, log}
}
